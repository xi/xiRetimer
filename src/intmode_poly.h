...
float** invertMatrix(float** A) {
  ...
  X[0][1]=(-1)*(A[0][1]*A[2][2]-(A[0][2]*A[2][1]));
  X[0][2]=A[0][1]*A[1][2]-(A[0][2]*A[1][1]);
     
  X[1][0]=(-1)*(A[1][0]*A[2][2]-A[1][2]*A[2][0]);
  X[1][1]=A[0][0]*A[2][2]-A[0][2]*A[2][0];
  X[1][2]=(-1)*(A[0][0]*A[1][2]-A[0][2]*A[1][0]);
 
  X[2][0]=A[1][0]*A[2][1]-A[1][1]*A[2][0];
  X[2][1]=(-1)*(A[0][0]*A[2][1]-A[0][1]*A[2][0]);
...
float int_poly(Marker* marker, float old) {
  ...
  float a=I[0][0]*d1*x1+I[0][1]*d2*x2+I[0][2]*(y2-y1);
  float b=I[1][0]*d1*x1+I[1][1]*d2*x2+I[1][2]*(y2-y1);
  float c=I[2][0]*d1*x1+I[2][1]*d2*x2+I[2][2]*(y2-y1);
