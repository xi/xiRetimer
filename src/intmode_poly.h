...
void _IntPoly::update(Marker* marker, int pi) {
	...
	float d2;
	if (IPi <= 0) {
		d1 = marker->getNew(1) - marker->getNew(0);
		d1 /= marker->getOld(1) - marker->getOld(0);
	} else {
		d1 = (marker->getNew(IPi - 1) - y2) / (marker->getOld(IPi - 1) - x2);
	}
	if (IPi >= IPn - 2) {
		d2 = marker->getNew(IPn - 1) - marker->getNew(IPn - 2);
		d2 /= marker->getOld(IPn - 1) - marker->getOld(IPn - 2);
	} else {
	...
	_IntPoly::d = y1;
	_IntPoly::d -= _IntPoly::a * x1 * x1 * x1;
	_IntPoly::d -= _IntPoly::b * x1 * x1;
	_IntPoly::d -= _IntPoly::c * x1;
